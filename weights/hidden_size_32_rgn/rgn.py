...
def nll_loss(phi, mu, sigma, targets):
    ...
    # Sign penalty (s.t. Phi and Target have the same sign)
    sign_mismatch = torch.relu(-phi * targets) # penalize when signs differ
    sign_penalty = 0.1 * torch.mean(sign_mismatch)

    return nll_loss + variance_penalty + sign_penalty
